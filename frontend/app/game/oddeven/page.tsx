...
import { playOddEven } from '@/lib/api';
...
export default function OddEvenPage() {
  ...
  const handlePlay = async () => {
    ...
    setTimeout(async () => {
      try {
        const data = await playOddEven(bet, choice);
        setDisplayNum(data.number);
        setResult(data);
        refreshUser();
      } catch (e: any) {
  ...
  return (
    <div style={{ background: '#0a0a0f', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '2rem 1rem' }}>
        <h1 style={{ fontSize: 28, fontWeight: 900, color: '#e2e2e2', marginBottom: 4 }}>🎲 홀짝</h1>
        <p style={{ color: '#888', marginBottom: 32, fontSize: 14 }}>1~100 숫자 — 홀수 또는 짝수, 1.9배 배당</p>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{
            ...
            <span style={{ fontSize: 52, fontWeight: 900, color: result ? (result.win ? '#4ade80' : '#f87171') : '#f0c040' }}>
              {displayNum ?? '?'}
            </span>
            {displayNum && !rolling && (
              <span style={{ fontSize: 13, color: '#888', marginTop: 2 }}>
                {displayNum % 2 === 0 ? '짝수' : '홀수'}
              </span>
            )}
          </div>
        </div>
        ...
        <button onClick={handlePlay} disabled={loading || !user} style={{
          ...
          {loading ? '뽑는 중...' : `🎲 ${bet.toLocaleString()}P 베팅`}
        </button>
        ...
        {result && !result.error && (
          <div style={{ background: result.win ? '#0a2010' : '#200a0a', border: `2px solid ${result.win ? '#4ade80' : '#f87171'}`, borderRadius: 12, padding: '1.2rem', textAlign: 'center' }}>
            ...
            <p style={{ color: '#888', fontSize: 13, margin: 0 }}>잔고: {result.newPoints?.toLocaleString()}P</p>
          </div>
        )}
        {result?.error && (
